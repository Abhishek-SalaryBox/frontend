...
export function PerformanceVisualization() {
  ...
  return (
    <Card className="w-full max-w-xl shadow-lg border-2">
      <CardContent className="p-6">
        <h2 className="text-xl font-semibold mb-4 text-center">Website Analysis Visualization</h2>
        <Tabs defaultValue="performance" className="w-full">
          ...
          <TabsContent value="performance" className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              {performanceData.slice(0, 2).map((item) => (
                <Card key={item.name} className="p-4">
                  ...
                  <Gauge
                    ...
                    size="medium"
                    showValue={false}
                    label={`${item.value} ${item.name === "CLS" ? "" : "ms"}`}
                    color={item.value <= item.limit ? "green" : "amber"}
                  />
                </Card>
              ))}

              {performanceData.slice(2, 4).map((item) => (
                <Card key={item.name} className="p-4">
                  <div className="text-center mb-2">
                    <span className="text-sm font-medium">{item.name}</span>
                  </div>
            ...
            {/* <div className="h-[180px] mt-6">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={performanceData} margin={{ top: 10, right: 10, left: 10, bottom: 20 }}>
